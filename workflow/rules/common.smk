...
if "twobit" not in ref_dict:
    ...
    twobit = "resources/" + "".join(os.path.basename(fasta).split(".")[:-1]) + ".2bit"
else:
    twobit = ref_dict["twobit"]

if "split_bed" not in ref_dict:
    region_length = ref_dict["split_region_length"]
    ref_split_bed = "resources/" + "".join(os.path.basename(twobit).split(".")[:-1]) + ".bed"
else:
...
def bbq_installation_check(wildcards):
    if "bbq" in config and "global" in config["bbq"] and "bbq_path" in config["bbq"]["global"]:
        ...
        return f"{"/".join(lookup_bbq_path().split("/")[:-1])}/{poetry_filename}"
    else: 
...
def aggregate_kmer_counts(wildcards):
    ...
    return expand("results/{sample}/kmers/{region}_kmers.txt", sample = samples.index.tolist(), region = regions)

def aggregate_var_calls(wildcards):
    regions = get_regions(wildcards)
    
    return expand("results/{sample}/variants/{region}_variants.vcf", sample = samples.index.tolist(), region = regions)
...
def get_var_calling_params(wildcards):
    ...
    if "no_positional_update" in config["bbq"]["variant_calling"] and config["bbq"]["variant_calling"]["no_positional_update"]: 
        extra += "--no_update "
