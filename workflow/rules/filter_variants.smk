...
rule plot_AF_coverage_muttypes:
    input:
        "results/{sample}/variants/variants_cov_filter_PASS.vcf",
    output:
        mut_type_counts = report("results/{sample}/plots/mut_type_counts.png", caption="../report/mut_type_counts.rst", category="Plots", subcategory="Variant calls: {sample}", labels={"Figure": "Mutation type counts"}),
        allele_frequency = report("results/{sample}/plots/allele_frequency.png", caption="../report/allele_frequency.rst", category="Plots", subcategory="Variant calls: {sample}", labels={"Figure": "Allele frequency"}),
        coverage = report("results/{sample}/plots/coverage.png", caption="../report/coverage.rst", category="Plots", subcategory="Variant calls: {sample}", labels={"Figure": "Coverage"}),
    resources: 
        mem_mb=lambda wildcards, attempt: attempt * 1024,
        runtime=lambda wildcards, attempt: attempt * 60,
    log:
